...
// scaled coordinates
fn dummy(x: i8, y: i8) -> bool {
    (x.abs() + y.abs()) % 2 == 1
}
fn cond_a1(x: i8, y: i8) -> bool {
    x - y >= -EDGE_SIZE * 2
}
fn cond_a2(x: i8, y: i8) -> bool {
    x - y <= EDGE_SIZE * 2
}
fn cond_b1(x: i8, y: i8) -> bool {
    x + y >= -EDGE_SIZE * 2
}
fn cond_b2(x: i8, y: i8) -> bool {
    x + y <= EDGE_SIZE * 2
}
fn cond_c1(_x: i8, y: i8) -> bool {
    y >= -EDGE_SIZE
}
fn cond_c2(_x: i8, y: i8) -> bool {
    y <= EDGE_SIZE
}
...
fn main() {
    ...
    for y in -offset_y..offset_y + 1 {
        for x in -offset_x..offset_x + 1 {
            ...
            // board labels
            map[(y + offset_y) as usize][(x + offset_x) as usize] = match trues {
                // dummy
                [true, _, _, _, _, _, _] => -1,
